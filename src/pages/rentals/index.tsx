...
type ColumnType = ColumnDef<RentalInterface, unknown>;
...
export function RentalListPage(props: RentalListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RentalInterface>>(
    () => `/rentals?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.rental_date ? format(parseISO(record?.rental_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    {
      id: 'return_date',
      header: 'Return Date',
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.return_date ? format(parseISO(record?.return_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    hasAccess('tool', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'tool',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/tools/view/${record.tool?.id}`}>
              {record.tool?.name}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/rentals/view/${record.id}`} passHref legacyBehavior>
                <Button
