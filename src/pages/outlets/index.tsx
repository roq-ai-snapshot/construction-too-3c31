...
type ColumnType = ColumnDef<OutletInterface, unknown>;

interface OutletListPageProps {
...
export function OutletListPage(props: OutletListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getOutlets({
        relations: ['company', 'tool.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['name.contains', 'address.contains', 'city.contains', 'state.contains', 'zip_code.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<OutletInterface>>(
    () => `/outlets?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'zip_code', header: 'Zip Code', accessorKey: 'zip_code' },
    ...
    hasAccess('tool', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Tool',
          accessorKey: 'tool',
          cell: ({ row: { original: record } }: any) => record?._count?.tool,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/outlets/view/${record.id}`} passHref legacyBehavior>
                <Button
